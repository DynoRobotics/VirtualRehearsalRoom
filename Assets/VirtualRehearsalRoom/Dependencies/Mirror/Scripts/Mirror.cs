...
public class Mirror : MonoBehaviour
{
    ...
    void UpdateCamera(ScriptableRenderContext SRC, Camera camera)
    {
            ...
            camera.tag != "PortalCam") // is the current camera eligeble for portalling?
        {
            ...
            bool stereoEnabled = camera.gameObject.name == "VR Camera";
            if (stereoEnabled)
            {
                try
                {
    ...
    private void CopyCameraProperties(Camera src, Camera dest)
    {
        ...
        dest.GetUniversalAdditionalCameraData().renderPostProcessing = false;
    }
