    ...
    private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
    {
        ...
        using (new EditorGUI.DisabledScope(readOnly))
        {
            ...
            else
            {
                ...
                buttonRect.x += buttonRect.width;
                if (DrawUtils.ButtonHelper(buttonRect, stateString, stateString + " In Build", EditorStyles.miniButtonMid, tooltipMsg))
                ...
                if (DrawUtils.ButtonHelper(buttonRect, "Remove...", "Remove from Build", EditorStyles.miniButtonRight, tooltipMsg))
