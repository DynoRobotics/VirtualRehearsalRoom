...
public class ActionBasedControllerManager : MonoBehaviour
{
    ...
    [SerializeField] 
    [Tooltip("Selects if the controller with a direct or ray interactor should be used.")]
    bool m_UseDirectController;
    /// <summary>
    /// Use controller direct instead of ray interactor
    /// </summary>
    public bool useDirectController
    {
        get => m_UseDirectController;
        set
        {
            m_UseDirectController = value;
            SetDirectController(value);
            SetBaseController(!value);
        }
    }
    ...
    public GameObject baseControllerGameObject
    {
        ...
        set 
        { 
            m_BaseControllerGameObject = value;
            FindBaseControllerComponents();
        }
    }

    [SerializeField, FormerlySerializedAs("m_DirectControllerGO")]
    [Tooltip("The direct controller GameObject, used for changing default settings on its components during state transitions.")]
    GameObject m_DirectControllerGameObject;
    /// <summary>
    /// The base controller <see cref="GameObject"/>, used for changing default settings on its components during state transitions.
    /// </summary>
    public GameObject directControllerGameObject
    {
        get => m_DirectControllerGameObject;
        set 
        { 
            m_DirectControllerGameObject = value;
            FindBaseControllerComponents();
        }
    }
...
#pragma warning restore IDE0044
    ...
    // Components of the base controller to switch on and off for different states
    XRBaseController m_BaseController;
    XRBaseInteractor m_BaseInteractor;
    XRInteractorLineVisual m_BaseLineVisual;

    // Components of the direct controller to switch on and off for different states
    XRBaseController m_DirectController;
    XRBaseInteractor m_DirectInteractor;
    ...
    void FindDirectControllerComponents()
    {
        if (m_DirectControllerGameObject == null)
        {
            Debug.LogWarning("Missing reference to Base Controller GameObject.", this);
            return;
        }

        if (m_DirectController == null)
        {
            m_DirectController = m_DirectControllerGameObject.GetComponent<XRBaseController>();
            if (m_DirectController == null)
                Debug.LogWarning($"Cannot find any {nameof(XRBaseController)} component on the Direct Controller GameObject.", this);
        }

        if (m_DirectInteractor == null)
        {
            m_DirectInteractor = m_BaseControllerGameObject.GetComponent<XRBaseInteractor>();
            if (m_DirectInteractor == null)
                Debug.LogWarning($"Cannot find any {nameof(XRBaseInteractor)} component on the Direct Controller GameObject.", this);
        }
    ...
    /// <summary>
    /// Find and configure the components on the direct controller.
    /// </summary>
    /// <param name="enable"> Set it true to enable the base controller, false to disable it. </param>
    void SetDirectController(bool enable)
    {
        FindDirectControllerComponents();

        if (m_DirectController != null)
            m_DirectController.enableInputActions = enable;
        
        if (m_DirectInteractor != null)
            m_DirectInteractor.enabled = enable;
    }
    ...
    void OnEnterSelectState(StateId previousStateId)
    {
        ...
        switch (previousStateId)
        {
            case StateId.None: 
                ...
                // Enable interactor controller components
                if (m_UseDirectController)
                {
                    SetDirectController(true);
                }
                else
                {
    ...
    void OnEnterInteractState(StateId previousStateId)
    {
        ...
        Debug.Log("Enabeling translate an/-chor");
